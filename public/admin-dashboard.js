...
function renderGrowthKpis(growth) {
  ...
  const kpis = [
    {
      label: '공구매출',
      value: formatWon(current.revenue),
      previous: formatWon(previous.revenue),
      change: growth.changes?.revenueRate,
      note: 'C × F × V'
    },
    {
      label: '구매 고객 수',
      value: `${formatNumber(current.activeCustomers)}명`,
      previous: `${formatNumber(previous.activeCustomers)}명`,
      change: growth.changes?.customerRate,
      note: '선택 기간 1회 이상 구매'
    },
    {
      label: '활성 고객당 참여일수',
      value: `${formatDecimal(current.avgParticipationDays)}일`,
      previous: `${formatDecimal(previous.avgParticipationDays)}일`,
      change: growth.changes?.frequencyRate,
      note: '고객별 서로 다른 공구일자'
    },
    {
      label: '활성 고객당 공구매출',
      value: formatWon(current.revenuePerActiveCustomer),
      previous: formatWon(previous.revenuePerActiveCustomer),
      change: null,
      note: '공구매출 / 구매 고객 수'
    },
    {
      label: '참여 1일당 주문금액',
      value: formatWon(current.revenuePerParticipationDay),
      previous: formatWon(previous.revenuePerParticipationDay),
      change: growth.changes?.valueRate,
      note: '공구매출 / 총 참여일수'
    },
    {
      label: '2회 이상 참여 고객',
      value: `${formatNumber(current.twoPlusCustomerCount)}명`,
      previous: `${formatRate(previous.twoPlusRate)}`,
      change: null,
      note: '기간 내 2일 이상 참여 비율'
    },
    {
      label: '신규 구매 고객',
      value: `${formatNumber(current.newCustomerCount)}명`,
      previous: '-',
      change: null,
      note: '전체 이력 최초 공구일자'
    },
    {
      label: '복귀 구매 고객',
      value: `${formatNumber(current.returningCustomerCount)}명`,
      previous: '-',
      change: null,
      note: '직전 기간 미구매 후 재구매'
    }
  ];

  els.growthKpis.innerHTML = kpis.map(item => `
    <article class="growth-kpi">
      <span>${escapeHtml(item.label)}</span>
      <strong>${escapeHtml(item.value)}</strong>
      <small>이전 ${escapeHtml(item.previous)}${item.change == null ? '' : ` · ${formatSignedRate(item.change)}`}</small>
...
function renderGrowthDrivers(growth) {
  ...
  const drivers = [
    ['customerRate', '구매 고객 수'],
    ['frequencyRate', '고객당 참여일수'],
    ['valueRate', '참여 1일당 주문금액']
  ];

  els.growthDrivers.innerHTML = drivers.map(([key, label]) => {
    const value = growth.changes?.[key];
    const isDominant = driver?.key && key.toLowerCase().includes(driver.key);
...
function renderCustomerMovement() {
  ...
  els.movementMatrix.innerHTML = `
    ...
    ${(movement.matrix || []).map(row => `
      ...
      ${(row.cells || []).map(cell => `
        <button class="movement-cell movement-button" type="button" data-customers="${encodeCustomerPayload(cell.customers)}" data-title="${escapeAttribute(`${row.previousLabel} → ${cell.currentLabel}`)}">
  ...
  els.movementMatrix.querySelectorAll('[data-customers]').forEach(button => {
    button.addEventListener('click', () => {
      openCustomerDrawer(button.dataset.title, decodeCustomerPayload(button.dataset.customers));
    });
  });
}
...
function renderLifecycleCards() {
  ...
  const cards = [
    ['신규 고객', lifecycle.newCustomers || []],
    ['유지 고객', lifecycle.retainedCustomers || []],
    ['복귀 고객', lifecycle.returningCustomers || []],
    ['관심 필요 고객', lifecycle.atRiskCustomers || []],
    ['휴면 고객', lifecycle.dormantCustomers || []],
    ['장기 휴면 고객', lifecycle.longDormantCustomers || []]
  ];

  els.lifecycleCards.innerHTML = cards.map(([label, customers]) => `
    <button class="lifecycle-card" type="button" data-title="${escapeAttribute(label)}" data-customers="${encodeCustomerPayload(customers)}">
      <span>${escapeHtml(label)}</span>
      <strong>${formatNumber(customers.length)}명</strong>
    </button>
  `).join('');

  els.lifecycleCards.querySelectorAll('[data-customers]').forEach(button => {
    button.addEventListener('click', () => {
      openCustomerDrawer(button.dataset.title, decodeCustomerPayload(button.dataset.customers));
    });
  });
...
function upsertStackedBarChart(chart, canvas, items, buckets, labelPrefix) {
  ...
  return new Chart(canvas, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: context => `${labelPrefix} ${context[0]?.label || ''}`,
            label: context => {
...
function openCustomerDrawer(title, customers) {
  state.drawerTitle = title || '고객 목록';
  state.drawerCustomers = Array.isArray(customers) ? customers : [];
  state.drawerQuery = '';
  els.customerDrawerTitle.textContent = state.drawerTitle;
...
function renderCustomerDrawerList() {
  ...
  els.customerDrawerList.innerHTML = customers.slice(0, 300).map(customer => `
    <tr>
...
function updateStatusLine() {
  ...
  setStatus(
    `${data.sheetName} · ${MODE_LABELS[state.mode] || data.mode} · 기준 ${getBasisLabel(data.basis)} · 그래프 ${formatNumber(pointCount)}구간 · 원본 유효 ${formatNumber(data.meta.totalValidRowCount)}행 · 통계 포함 ${formatNumber(data.meta.validRowCount)}행 · 오늘 제외 ${formatNumber(data.meta.excludedTodayRowCount)}행 · 아군 제외 ${formatNumber(data.meta.excludedAllyRowCount)}행 · 성장분석 포함 ${formatNumber(data.meta.growthAnalyzedRowCount)}행 · 확인 필요 ${formatNumber(data.meta.warningCount)}건`
  );
}
...
function encodeCustomerPayload(customers) {
  try {
    return btoa(unescape(encodeURIComponent(JSON.stringify(customers || []))));
  } catch {
    return '';
  }
}

function decodeCustomerPayload(value) {
  try {
