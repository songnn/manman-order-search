...
test('TV 픽업 페이지는 실제 화면 전체를 단일 상품판으로 채운다', async () => {
  ...
  assert.match(css, /\.board-column\s*\{[\s\S]*?grid-template-rows:\s*var\(--help-ticker-height\) minmax\(0,\s*1fr\);/);
...
test('픽업 도움 전화번호를 최상단 무한 뉴스티커로 크게 안내한다', async () => {
  const [html, css] = await pickupFiles();
  const tickerIndex = html.indexOf('class="pickup-help-ticker"');
  const headerIndex = html.indexOf('class="board-header"');
  const zonesIndex = html.indexOf('id="zonesLayout"');

  assert.match(html, /class="pickup-help-ticker"[\s\S]*?role="note"[\s\S]*?010\.9394\.9071/);
  assert.ok(tickerIndex >= 0);
  assert.ok(tickerIndex < headerIndex);
  assert.ok(tickerIndex < zonesIndex);
...
test('세 보관존 위치를 폭에 맞춰 변하는 고대비 안내 레일로 표시한다', async () => {
  const [html, css, js] = await pickupFiles();
  ...
  assert.equal((html.match(/class="zone-direction"/g) || []).length, 3);
  assert.equal((html.match(/class="zone-direction__spot"[\s\S]*?role="note"/g) || []).length, 3);
  assert.match(css, /--zone-compact-header-height:\s*88px;/);
  assert.match(css, /--zone-compact-block-chrome:\s*101px;/);
  assert.match(css, /\.zone\s*\{[\s\S]*?container-name:\s*pickup-zone;[\s\S]*?container-type:\s*inline-size;/);
...
test('날짜·상품 요약·페이지 헤더는 화면에서 숨기고 복구 가능한 상태로 보존한다', async () => {
  ...
  assert.match(html, /복구용 보관:[\s\S]*?<header[\s\S]*?class="board-header"[\s\S]*?hidden[\s\S]*?data-restore-note="[^"]*복구용"[\s\S]*?id="pickupDate"[\s\S]*?id="summaryCards"[\s\S]*?id="pageIndicator"[\s\S]*?<\/header>/);
  assert.match(css, /\.board-header\[hidden\]\s*\{[\s\S]*?display:\s*none;/);
...
test('보관존은 상품 수에 맞춘 2차원 타일을 쓰고 마지막 행까지 균등하게 나눈다', async () => {
  ...
  assert.match(css, /\.product-grid\s*\{[\s\S]*?align-content:\s*start;/);
