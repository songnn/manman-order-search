...
test('주문조회 네 단계 캡처와 점장 전화번호가 QR 없이 표시된다', async () => {
  ...
  assert.match(html, /언제든지 아래 번호로 연락주시면 친절히 안내드리겠습니다\./);
  assert.doesNotMatch(html, /QR|qr-code|qrcode/i);
  assert.match(html, /내 주문 확인[\s\S]*?카운터 결제[\s\S]*?픽업존 수령/);
  assert.match(html, /TV 오른쪽 끝/);
});

test('숫자 요약 없이 짧은 헤더에서 보관 위치를 우선 안내한다', async () => {
  const html = await readFile(path.join(root, 'public', 'tv-pickup.html'), 'utf8');
  const css = await readFile(path.join(root, 'public', 'tv-pickup.css'), 'utf8');
  const js = await readFile(path.join(root, 'public', 'tv-pickup.js'), 'utf8');

  assert.doesNotMatch(html, /summaryCards|ambientCount|chilledCount|frozenCount/);
  assert.match(html, /상온[\s\S]*?TV 바로 우측 선반대/);
  assert.match(html, /냉장[\s\S]*?TV 오른쪽 끝/);
  assert.match(html, /냉동[\s\S]*?뒤쪽 5시 방향/);
  assert.match(css, /\.board-column\s*\{[\s\S]*?grid-template-rows:\s*50px minmax\(0,\s*1fr\) 34px;/);
  assert.match(css, /\.zone\s*\{[\s\S]*?grid-template-rows:\s*42px minmax\(0,\s*1fr\);/);
  assert.match(css, /\.zone-header\s*\{[\s\S]*?display:\s*flex;[\s\S]*?align-items:\s*center;/);
  assert.doesNotMatch(js, /renderSummary|입고 대기|입고 확인 중|readyByStorage/);
});
