...
test('주문조회 네 단계 캡처와 점장 전화번호가 QR 없이 표시된다', async () => {
  ...
  assert.match(html, /TV 오른쪽 끝 냉장 픽업존/);
  assert.match(html, /TV 뒤쪽 4시 방향 냉동 픽업존/);
});
...
test('픽업존 헤더는 보관방법·수량을 붙이고 위치 안내를 오른쪽에 크게 표시한다', async () => {
  ...
  assert.match(html, /냉장[\s\S]*?chilledCount[\s\S]*?→ TV 오른쪽 끝 냉장 픽업존/);
  assert.match(html, /냉동[\s\S]*?frozenCount[\s\S]*?↘ TV 뒤쪽 4시 방향 냉동 픽업존/);
