...
test('연락처는 구분선과 라벨 없이 잠실체의 큰 번호로 표시된다', async () => {
  const css = await readFile(path.join(root, 'public', 'tv-pickup.css'), 'utf8');
...
test('TV 픽업 화면의 모든 글자는 로컬 잠실체를 우선 사용한다', async () => {
  ...
  assert.ok(fontFamilies.length > 0);
  assert.ok(fontFamilies.every(fontFamily => fontFamily.includes("'The Jamsil'")));
