...
test('주문조회 네 단계 캡처와 점장 전화번호가 QR 없이 표시된다', async () => {
  ...
  assert.match(html, /TV 뒤쪽 4시 방향[\s\S]*?냉동[\s\S]*?zone-direction__number">1<[\s\S]*?픽업존/);
});
...
test('연락처는 구분선과 라벨 없이 강조용 잠실체의 큰 번호로 표시된다', async () => {
  ...
  assert.match(css, /\.contact-copy strong\s*\{[\s\S]*?font-size:\s*26px;/);
  assert.match(css, /\.contact-copy small\s*\{[\s\S]*?font-size:\s*16px;/);
  assert.match(contactPhone, /font-family:\s*var\(--font-highlight\);/);
  assert.match(contactPhone, /text-align:\s*left;/);
  assert.match(contactPhone, /font-size:\s*48px;/);
...
test('픽업존 헤더는 보관방법·수량을 붙이고 위치 안내를 오른쪽에 크게 표시한다', async () => {
  ...
  assert.match(html, /상온[\s\S]*?ambientCount[\s\S]*?→ TV 우측[\s\S]*?선반대 픽업존/);
  assert.match(html, /냉장[\s\S]*?chilledCount[\s\S]*?→ TV 오른쪽 끝[\s\S]*?냉장[\s\S]*?zone-direction__number">1<[\s\S]*?픽업존/);
  assert.match(html, /냉동[\s\S]*?frozenCount[\s\S]*?↘ TV 뒤쪽 4시 방향[\s\S]*?냉동[\s\S]*?zone-direction__number">1<[\s\S]*?픽업존/);
  ...
  assert.match(css, /\.zone-direction\s*\{[\s\S]*?justify-self:\s*end;[\s\S]*?display:\s*flex;[\s\S]*?text-align:\s*right;/);
  assert.match(css, /\.zone-direction__spot\s*\{[\s\S]*?linear-gradient[\s\S]*?font-size:\s*28px;/);
  assert.match(css, /\.zone-direction__number\s*\{[\s\S]*?width:\s*29px;[\s\S]*?height:\s*29px;[\s\S]*?border-radius:\s*7px;/);
...
test('좌우 상단과 안내·보관 헤더는 같은 높이와 시작선을 공유한다', async () => {
  ...
  assert.match(css, /\.date-meta\s*\{[\s\S]*?gap:\s*0;/);
  assert.match(css, /\.date-meta small\s*\{[\s\S]*?font-size:\s*24px;/);
  assert.match(css, /\.date-meta > span\s*\{[\s\S]*?font-size:\s*16px;/);
  assert.match(css, /\.summary-card img\s*\{[\s\S]*?width:\s*44px;[\s\S]*?height:\s*44px;/);
  assert.match(css, /\.summary-card strong\s*\{[\s\S]*?font-size:\s*24px;[\s\S]*?font-weight:\s*600;/);
  assert.match(css, /\.summary-card span\s*\{[\s\S]*?font-size:\s*13px;/);
});
...
test('실제 만만 로고와 작은 총·보관방법별 상품 요약을 표시한다', async () => {
  ...
  assert.match(css, /\.summary-card\s*\{[\s\S]*?height:\s*52px;[\s\S]*?grid-template-columns:\s*44px auto;/);
...
test('안내 단계·수령 흐름·문의 영역은 현대적인 카드와 균일한 패딩을 사용한다', async () => {
  ...
  assert.match(css, /\.guide-step > p b\s*\{[\s\S]*?border-radius:\s*10px;[\s\S]*?linear-gradient[\s\S]*?font-size:\s*20px;/);
  assert.doesNotMatch(css, /\.guide-step > p b::before/);
...
test('상품카드는 각 보관존 폭을 채우고 정사각 상단 이미지·강제 두 줄 말줄임을 사용한다', async () => {
  ...
  assert.match(css, /\.product-card__name-text\s*\{[\s\S]*?max-height:\s*32px;[\s\S]*?font-size:\s*14px;[\s\S]*?font-weight:\s*500;[\s\S]*?text-align:\s*center;[\s\S]*?-webkit-line-clamp:\s*2;/);
