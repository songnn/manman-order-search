...
test('다크 배지는 세 카드 경로에서 우측 반투명 사각 태그로 표시된다', async () => {
  ...
  assert.match(html, /\.storage-method-badge\.storage-method-badge--theme-dark\s*\{[\s\S]*?left:\s*auto;[\s\S]*?right:\s*12px;[\s\S]*?border-radius:\s*8px;[\s\S]*?background:\s*#0f1216c2;[\s\S]*?color:\s*#fff;/);
  ...
  assert.match(html, /\.orders-section\.show-all \.storage-method-badge\.storage-method-badge--theme-dark\s*\{[\s\S]*?right:\s*5px;[\s\S]*?height:\s*20px;[\s\S]*?border-radius:\s*5px;/);
  assert.match(html, /\.schedule-product-thumb-wrap \.storage-method-badge\.storage-method-badge--theme-dark\s*\{[\s\S]*?right:\s*4px;[\s\S]*?min-height:\s*22px;[\s\S]*?border-radius:\s*6px;/);
