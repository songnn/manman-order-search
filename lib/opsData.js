...
export async function getOperationsDashboardData() {
  const [inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, settlementRows] = await Promise.all([
  ...
  return {
    ...
    settlementReview: buildSettlementReview(inventoryRows, settlementRows),
    generatedAt: new Date().toISOString()
  };
}

function buildSettlementReview(inventoryRows = [], settlementRows = []) {
  ...
  inventoryRows.forEach(row => {
    ...
    const current = inventoryByProductDate.get(key) || {
      productKey,
    ...
    inventoryByProductDate.set(key, current);
  });
  ...
  const items = settlementRows.map(row => {
    ...
    const category = classifySettlementItem(row, inventory);
    ...
    return {
      ...
      isFreshProduce: Boolean(row.is_fresh_produce),
      salesTypes,
      storageMethods: Array.from(inventory?.storageMethods || []),
      imageUrl: inventory?.imageUrl || imageByProduct.get(productKey) || '',
      needsGeneratedImage: !(inventory?.imageUrl || imageByProduct.get(productKey)),
...
function classifySettlementItem(row, inventory) {
  ...
  if (row.is_fresh_produce || /과일|야채|채소|농산|복숭아|수박|사과|포도|망고|토마토|버섯|참외|자두|오이|상추|감자|고구마|양파|파프리카/.test(productName)) {
    return '과일·야채';
  }
