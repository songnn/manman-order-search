...
export async function syncInventorySheets() {
  ...
  const records = inventoryRows.map(row => {
    ...
    const hqBufferQuantity = row.hasHqBufferQuantity
...
export async function getOperationsDashboardData(options = {}) {
  ...
  const normalizedInventoryRows = inventoryRows.map(normalizeInventoryRowProductFields);
  const normalizedInventoryRawRows = inventoryRawRows.map(normalizeInventoryRowProductFields);
  ...
  const settlementOrderImageRows = settlementPickupDateRange
    ...
    : [];
  const orderPickupDateValues = getInventoryPickupDateValues(normalizedInventoryRows);
...
function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, orderAggregates) {
  ...
  return inventoryRows.map(row => {
    ...
    const customerOrderQuantity = inventoryOrderQuantity == null
      ? Number(orderInfo.orderQuantity || 0)
      : Number(inventoryOrderQuantity || 0);
    const productCode = getRawStringByHeaders(row.raw_json, INVENTORY_RAW_HEADERS.productCode);
...
function resolveDisplayProductName(currentName, rawName, rawCode = '') {
  const name = clean(currentName);
  const candidate = clean(rawName);
