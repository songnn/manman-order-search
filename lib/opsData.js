...
export async function getOperationsDashboardData() {
  ...
  return {
    ...
    latestInboundItems: inboundInventoryItems
      ...
      .sort(sortInboundItems),
    staleItems: staleInventoryItems
      .filter(item =>
        item.inboundDateKey &&
...
export async function completeStorageRequestEvent({ eventId, locationMemo, visitDateText, requestMemo }) {
  const event = await getStorageRequestEventById(eventId);
  ...
  const memoPayload = {
    ...
    locationMemo: clean(locationMemo) || parsed.locationMemo,
    visitDateText: clean(visitDateText) || parsed.visitDateText,
    requestMemo: clean(requestMemo) || parsed.requestMemo,
    completedAt
  };
