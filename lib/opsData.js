...
export async function getOperationsDashboardData() {
  ...
  const [orderRows, pickupCompletionRows] = await Promise.all([
    orderPickupDateValues.length
...
function buildOrderAggregates(orderRows, pickupCompletionRows) {
  ...
  (orderRows || []).forEach(row => {
    ...
    if (completed) {
      ...
      addCustomerQuantity(current.completedCustomers, customerLabel, quantity, completion?.completed_at);
    } else {
...
function addCustomerQuantity(map, customerLabel, quantity, completedAt = '') {
  ...
  if (completedAt && String(completedAt) > String(current.lastCompletedAt || '')) {
...
function mapToCustomerRows(map) {
  return Array.from(map.entries())
    .map(([customerLabel, value]) => ({
      customerLabel,
      quantity: Number(value?.quantity || value || 0),
      lastCompletedAt: value?.lastCompletedAt || ''
    }))
    .sort((a, b) => {
      if (b.quantity !== a.quantity) return b.quantity - a.quantity;
      if (String(b.lastCompletedAt || '') !== String(a.lastCompletedAt || '')) {
