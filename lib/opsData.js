...
export async function syncInventorySheets() {
  ...
  if (records.length) {
    await archiveInventoryRowsWithChangedStableIds(records);
...
export async function getOperationsDashboardData(options = {}) {
  ...
  const [inventoryRows, inventoryRawRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, settlementRows, generatedImagePaths] = await Promise.all([
    ...
    includeSettlement
      ...
      : Promise.resolve([]),
    includeSettlement ? readGeneratedProductImagePaths() : Promise.resolve(new Set())
  ]);
  ...
  const orderRows = orderPickupDateValues.length
    ? await readSupabaseRowsPaged('order_cache', query =>
        query
          .select('customer_label,product_name,quantity,order_date_text,pickup_date_text,pickup_date_value,source_row_number')
          .eq('store_name', OPS_CONFIG.STORE_NAME)
...
function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, orderAggregates) {
  ...
  return inventoryRows.map(row => {
    ...
    return {
      ...
      orderCustomers: Array.isArray(orderInfo.customers) ? orderInfo.customers : [],
      countedQuantity,
      isReceivingComplete,
