...
export async function getOperationsDashboardData() {
  const [inventoryRows, bufferEvents, receivingEvents, receivingChecks, orderRows, pickupCompletionRows] = await Promise.all([
    ...
    readSupabaseRowsPaged('order_cache', query =>
      query
        .select('source_sheet_name,source_row_number,customer_label,customer_digits4,product_name,quantity,pickup_date_text,pickup_date_value')
        .eq('store_name', OPS_CONFIG.STORE_NAME)
    ),
    readSupabaseRowsPaged('pickup_completions', query =>
      query
        .select('source_sheet_name,source_row_number,customer_label,customer_digits4,product_name,completed,completed_at')
        .eq('store_name', OPS_CONFIG.STORE_NAME)
    ).catch(error => {
      if (isMissingOpsTableError(error)) return [];
      throw error;
    })
  ]);
  const orderAggregates = buildOrderAggregates(orderRows, pickupCompletionRows);
  ...
  const salesInventoryItems = dashboardItems
    .filter(item => /^입고리스트/.test(item.sourceSheetName || ''));
  ...
  return {
    ...
    inboundItems: dashboardItems
      .filter(item => item.sourceSheetName === '입고리스트' && item.inboundDateKey === todayKey)
      .sort(sortInboundItems),
    latestInboundItems: dashboardItems
      .filter(item => item.sourceSheetName === '입고리스트' && item.inboundDateKey === latestInboundDate)
      .sort(sortInboundItems),
    staleItems: dashboardItems
      .filter(item => item.sourceSheetName === '입고리스트(D-2)')
      .sort(sortInboundItems),
...
export async function findCustomerCandidatesByDigits({ digits }) {
  ...
  if (!/^\d{4}$/.test(normalizedDigits)) {
    throw new Error('휴대폰 끝 4자리를 입력해주세요.');
  }

  const rows = await readSupabaseRows('order_cache', query =>
    query
      .select('customer_label,customer_digits4,pickup_date_value')
      .eq('store_name', OPS_CONFIG.STORE_NAME)
...
function buildOrderAggregates(orderRows, pickupCompletionRows) {
  const completionBySource = new Map();
  const aggregates = new Map();

  (pickupCompletionRows || []).forEach(row => {
    const sourceKey = sourceRowKey(row.source_sheet_name, row.source_row_number);
    if (!sourceKey) return;
    completionBySource.set(sourceKey, row);
  });
  ...
  (orderRows || []).forEach(row => {
    ...
    const current = aggregates.get(key) || {
      orderQuantity: 0,
      pickupCompletedQuantity: 0,
      customerCount: 0,
      completedCustomers: new Map(),
      pendingCustomers: new Map()
    };
    const quantity = Number(row.quantity || 0);
    const customerLabel = clean(row.customer_label);
    const sourceKey = sourceRowKey(row.source_sheet_name, row.source_row_number);
    const completion = completionBySource.get(sourceKey);
    ...
    if (completed) {
      current.pickupCompletedQuantity += quantity;
      addCustomerQuantity(current.completedCustomers, customerLabel, quantity);
    } else {
      addCustomerQuantity(current.pendingCustomers, customerLabel, quantity);
    }
  ...
  aggregates.forEach((aggregate, key) => {
    result.set(key, {
      ...
      pickupCompletedQuantity: aggregate.pickupCompletedQuantity,
      customerOrderRemainingQuantity: Math.max(
        0,
        aggregate.orderQuantity - aggregate.pickupCompletedQuantity
      ),
      customerCount: aggregate.customerCount,
      pickupCompletedCustomers: mapToCustomerRows(aggregate.completedCustomers),
      pickupPendingCustomers: mapToCustomerRows(aggregate.pendingCustomers)
    });
  });
...
function addCustomerQuantity(map, customerLabel, quantity) {
  const label = clean(customerLabel) || '고객';
  map.set(label, Number(map.get(label) || 0) + Number(quantity || 0));
}

function mapToCustomerRows(map) {
  return Array.from(map.entries())
    .map(([customerLabel, quantity]) => ({
      customerLabel,
      quantity
    }))
    .sort((a, b) => {
...
function sourceRowKey(sheetName, rowNumber) {
  const sourceSheetName = clean(sheetName);
  const sourceRowNumber = Number(rowNumber || 0);
  if (!sourceSheetName || !sourceRowNumber) return '';
...
function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks, orderAggregates) {
  ...
  return inventoryRows.map(row => {
    ...
    const pickupCompletedQuantity = Number(orderInfo.pickupCompletedQuantity || 0);
    const customerOrderRemainingQuantity = Math.max(
      0,
      Number(orderInfo.customerOrderRemainingQuantity || customerOrderQuantity - pickupCompletedQuantity)
    );
    const physicalRemainingQuantity = Math.max(
      0,
      inboundQuantity - bufferUsedQuantity - pickupCompletedQuantity
    );
    ...
    return {
      ...
      customerOrderRemainingQuantity,
      customerCount: Number(orderInfo.customerCount || 0),
      pickupCompletedCustomers: orderInfo.pickupCompletedCustomers || [],
      pickupPendingCustomers: orderInfo.pickupPendingCustomers || [],
      countedQuantity,
      isReceivingComplete,
      completedAt: receivingCheck?.completed_at || '',
      dDayOffset: row.d_day_offset,
      bufferHistory: (bufferHistoryByItem.get(row.stable_id) || []).slice(0, 8).map(event => ({
