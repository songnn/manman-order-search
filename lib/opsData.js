...
export async function syncInventorySheets() {
  ...
  const [imageMap, bufferNotes, settlementBufferMap, inventoryRows, rawRows] = await Promise.all([
    readProductImageMap(),
  ...
  const records = inventoryRows.map(row => {
    ...
    const settlementInfo = getAggregateForProductDate(settlementBufferMap, productKey, row.inboundDateKey);
...
export async function getOperationsDashboardData() {
  ...
  const latestInboundDate = dashboardItems
  ...
  return {
    ...
    todayKey,
    latestInboundDate,
    salesItems: dashboardItems
      .filter(item => item.initialBufferQuantity > 0 || item.bufferRemainingQuantity > 0)
      .sort(sortSalesItems),
...
function parseSettlementSheetRows(file, sheetName, rows, syncRunId) {
  const headerIndex = findHeaderRowIndex(rows, ['상품', '공급', '판매']);
  if (headerIndex < 0) return [];

  const headers = rows[headerIndex].map(normalizeHeader);
  const columns = {
    productName: findHeaderIndex(headers, ['상품명', '품명', '상품']),
    ...
    supplyVatIncluded: findHeaderIndex(headers, ['vat포함', 'vat 포함', '공급가포함', '공급가(vat포함)', '공급가']),
    salePrice: findHeaderIndex(headers, ['판매가', '매출가']),
    hqBuffer: findHeaderIndex(headers, ['본사버퍼', '추가수량', '버퍼'])
  };

  if (columns.productName < 0) return [];

  const records = [];
  const inferredDate = inferDateKeyFromText(`${file.name || ''} ${sheetName}`);
...
async function readRawOrderBufferNotes(syncRunId) {
  ...
  return (response.data.values || []).flatMap((row, index) => {
    ...
    if (!productName || !noteText || parsedBufferQuantity <= 0) return [];
...
async function readSettlementBufferAggregate() {
  ...
  rows.forEach(row => {
    const key = aggregateKey(row.product_key, row.settlement_date || '');
    const productKey = aggregateKey(row.product_key, '');
    ...
    const productOnly = map.get(productKey) || {
      quantity: 0,
      hqBufferQuantity: 0,
      supplyPriceVatIncluded: 0,
      salePrice: 0
    ...
    productOnly.quantity += Number(row.hq_buffer_quantity || 0);
    productOnly.hqBufferQuantity += Number(row.hq_buffer_quantity || 0);
    productOnly.supplyPriceVatIncluded = Number(row.supply_price_vat_included || productOnly.supplyPriceVatIncluded || 0);
    productOnly.salePrice = Number(row.sale_price || productOnly.salePrice || 0);

    map.set(productKey, productOnly);
  });
...
function getAggregateForProductDate(map, productKey, dateKey) {
  return map.get(aggregateKey(productKey, dateKey || '')) || map.get(aggregateKey(productKey, '')) || null;
}

function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks) {
  ...
  return inventoryRows.map(row => {
    ...
    return {
      ...
      inboundQuantity,
      remainingQuantity: Math.max(0, inboundQuantity - bufferUsedQuantity),
...
function parseBufferNoteQuantity(value) {
  ...
  const allocationMatch = text.match(/버퍼\s*배분[^\d]*(\d+(?:\s*[-~>→]+\s*\d+)*)/i);
  if (allocationMatch) {
    const numbers = allocationMatch[1].match(/\d+/g) || [];
