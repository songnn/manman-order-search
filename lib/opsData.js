...
export async function getOperationsDashboardData() {
  const [inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, settlementRows, generatedImagePaths] = await Promise.all([
  ...
  const orderRows = orderPickupDateValues.length
    ? await readSupabaseRowsPaged('order_cache', query =>
        query
          .select('customer_label,product_name,quantity,pickup_date_text,pickup_date_value')
          .eq('store_name', OPS_CONFIG.STORE_NAME)
  ...
  const inboundInventoryItems = dashboardItems
    .filter(item => normalizeSheetName(item.sourceSheetName) === '입고리스트');
  const staleInventoryItems = dashboardItems
    .filter(item => /^입고리스트/.test(normalizeSheetName(item.sourceSheetName)))
    .filter(isGroupSaleItem);
  const salesInventoryItems = inboundInventoryItems
    .filter(isGroupSaleItem);
  ...
  const staleAllItems = staleInventoryItems
    .filter(item =>
      item.inboundDateKey &&
      item.inboundDateKey >= staleHistoryStartDateKey &&
      item.inboundDateKey <= staleCutoffDateKey
    )
    .sort(sortInboundItems);
  const staleItems = staleAllItems
    .filter(item => item.inboundDateKey === staleCutoffDateKey)
    .sort(sortInboundItems);
  ...
  return {
    ...
    salesItems: salesInventoryItems
      ...
      .sort(sortInboundItems),
    inboundItems: inboundInventoryItems
      .filter(item => item.inboundDateKey === todayKey)
      .sort(sortInboundItems),
    latestInboundItems: inboundInventoryItems
      .filter(item => item.inboundDateKey === latestInboundDate)
      .sort(sortInboundItems),
    staleItems,
    staleDateGroups: buildStaleDateGroups(staleAllItems),
    settlementReview: buildSettlementReview(inventoryRows, settlementRows, generatedImagePaths),
...
export async function generateSettlementProductImages({ limit = 8, category = '', dryRun = false } = {}) {
  ...
  for (const target of targets) {
    try {
      ...
      const path = productImageStoragePath(target.productKey);
      ...
      generated.push({
        ...
        productKey: target.productKey,
        category: target.category,
        imageUrl: getProductImagePublicUrl(target.productKey),
        model: result.model,
        path
      });
    } catch (error) {
...
function getSettlementImageTargets(items = [], category = '') {
  ...
  return (items || [])
    ...
    .filter(item => {
      if (!item.productKey || seen.has(item.productKey)) return false;
...
function buildSettlementReview(inventoryRows = [], settlementRows = [], generatedImagePaths = new Set()) {
  ...
  inventoryRows.forEach(row => {
    ...
    if (!dateKey) return;
    addInventoryReviewAggregate(inventoryByProductDate, productKey, dateKey, row);
  });

  const items = settlementRows.map(row => {
    const productKey = row.product_key || normalizeProductKey(row.product_name);
    const dateKey = row.settlement_date || '';
    const inventory = inventoryByProductDate.get(aggregateKey(productKey, dateKey));
    ...
    const inventoryQuantity = Number(inventory?.inboundQuantity || 0);
    const diffQuantity = inventory
      ? settlementQuantity - inventoryQuantity
      : settlementQuantity;
    const salesTypes = Array.from(productInventory?.salesTypes || []);
    const category = classifySettlementItem(row, productInventory);
    const generatedImageUrl = hasGeneratedProductImage(generatedImagePaths, productKey)
      ? getProductImagePublicUrl(productKey)
      : '';
    const imageUrl = inventory?.imageUrl || imageByProduct.get(productKey) || generatedImageUrl;
    ...
    return {
      ...
      inventoryQuantity,
      diffQuantity,
      hasInventoryMatch: Boolean(inventory),
      mismatch: !inventory || Math.abs(diffQuantity) > 0.0001,
      supplyPriceExVat: Number(row.supply_price_ex_vat || 0),
      supplyPriceVatIncluded: Number(row.supply_price_vat_included || 0),
      salePrice: Number(row.sale_price || 0),
      hqBufferQuantity: Number(row.hq_buffer_quantity || 0),
      isFreshProduce: isFreshProduceSettlementItem(row),
      salesTypes,
      storageMethods: Array.from(productInventory?.storageMethods || []),
      imageUrl,
      generatedImageUrl,
      needsGeneratedImage: !imageUrl,
...
function buildProductImagePrompt(item) {
  const categoryHint = item.category === '과일·야채'
    ? 'Fresh produce photo that clearly matches the named fruit or vegetable.'
    : item.category === '상시'
...
async function generateOpenAIProductImage(prompt) {
  ...
  const fallbackModel = preferredModel === 'gpt-image-1.5' ? 'gpt-image-1' : 'gpt-image-1.5';
...
function resolveOpenAIImageModel() {
  const model = clean(process.env.OPENAI_IMAGE_MODEL);
  if (!model || /mini/i.test(model)) return 'gpt-image-1.5';
  return model;
}
...
function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, orderAggregates) {
  const bufferAdjustmentEvents = bufferEvents.filter(event => !isStorageRequestEvent(event));
  const storageRequestEvents = bufferEvents.filter(isStorageRequestEvent);
...
function isGroupSaleItem(item) {
  return normalizeSalesType(item?.salesType).includes('공구');
}
