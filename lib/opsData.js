...
export async function syncInventorySheets() {
  ...
  const imageMap = await readCachedProductImageMap(inventoryRows);
...
export async function generateSettlementProductImages({ limit = 40, category = '', dryRun = false } = {}) {
  const normalizedLimit = Math.max(1, Math.min(80, Number(limit || 40)));
...
function buildSettlementReview(
  ...
  orderImageRows.forEach(row => {
    ...
    if (!productKey || !imageUrl || imageByProduct.has(productKey)) return;
  ...
  const items = settlementRows.map(row => {
    ...
    const diffQuantity = inventory
      ? settlementQuantity - inventoryListQuantity
      : settlementQuantity;
    ...
    return {
      ...
      hasRawInventoryMatch: Boolean(rawInventory),
...
function resolveOpenAIImageModel() {
  ...
  if (!model || /mini/i.test(model)) return 'gpt-image-1';
  return model;
}
...
async function readCachedProductImageMap(inventoryRows = []) {
  ...
  rows.forEach(row => {
    ...
    const productKey = normalizeProductKey(productName);
    const imageUrl = clean(row.image_url);
    if (!productKey || !imageUrl || map.has(productKey)) return;
