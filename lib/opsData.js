...
function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks, orderAggregates) {
  const bufferAdjustmentEvents = bufferEvents.filter(event => event.event_source !== 'storage_request');
  const storageRequestEvents = bufferEvents.filter(event => event.event_source === 'storage_request');
