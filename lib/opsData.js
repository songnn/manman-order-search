...
export async function syncInventorySheets() {
  ...
  if (records.length) {
    await deleteInventoryRowsWithChangedStableIds(records);
...
async function readRawOrderBufferNotes(syncRunId) {
  ...
  const rows = (response.data.values || []).flatMap((row, index) => {
    ...
    const productName = cleanNonFormula(row[5]);
...
function buildDashboardItems(inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes, orderAggregates) {
  ...
  return inventoryRows.map(row => {
    ...
    return {
      ...
      salePrice: Number(row.sale_price || 0),
      imageUrl: row.image_url || OPS_CONFIG.IMAGE_FALLBACK_URL,
...
async function deleteInventoryRowsWithChangedStableIds(records = []) {
  ...
  const changedStableIds = new Set();
    ...
    for (let i = 0; i < sourceRowNumbers.length; i += 500) {
      ...
      (data || []).forEach(row => {
        ...
        if (nextStableId && row.stable_id !== nextStableId) {
          changedStableIds.add(row.stable_id);
        }
      });
    }
  }

  const ids = [...changedStableIds].filter(Boolean);
  for (let i = 0; i < ids.length; i += 500) {
    const { error } = await supabaseAdmin
      .from('operations_inventory_items')
      .delete()
      .eq('store_name', OPS_CONFIG.STORE_NAME)
      .in('stable_id', ids.slice(i, i + 500));
