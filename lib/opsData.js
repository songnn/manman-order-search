...
export async function getOperationsDashboardData() {
  const [inventoryRows, bufferEvents, receivingEvents, receivingChecks, bufferNotes] = await Promise.all([
...
export async function updateStorageRequestEvents({
  ...
  const { data: events, error: readError } = await supabaseAdmin
    ...
    .eq('store_name', OPS_CONFIG.STORE_NAME)
    .in('event_id', ids)
    .in('event_source', ['storage_request_pending', 'storage_request']);
  ...
  for (const event of events) {
    ...
    const memoPayload = {
      ...
      completedAt: normalizedStatus === 'completed' ? (parsed.completedAt || completedAt) : ''
    };
  ...
  return inventoryRows.map(row => {
    ...
    return {
      ...
      storageRequests: (storageRequestByItem.get(row.stable_id) || []).slice(0, 80).map(event => ({
        ...
        eventSource: event.event_source || '',
        status: event.event_source === 'storage_request_pending' ? 'pending' : 'completed',
        createdAt: event.created_at
      })),
...
function isStorageRequestEvent(event) {
  return ['storage_request', 'storage_request_pending'].includes(event?.event_source || '');
}
