  ...
  orders
    ...
    .forEach(order => {
      ...
      if (order.customerName) {
        item.recentBuyers.unshift(maskCustomerLabel(order.customerName));
      }
      stats.set(key, item);
    });
...
function maskCustomerLabel(label) {
  const raw = normalizeCustomerLabel(label).replace(/\d{4}$/, '').trim();
  if (!raw) return '고객님';
  if (raw.length <= 2) return `${raw[0]}*님`;

  return `${raw.slice(0, 1)}${'*'.repeat(Math.min(2, raw.length - 1))}${raw.slice(-1)}님`;
}
