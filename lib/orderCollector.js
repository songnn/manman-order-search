...
async function writeRowsToRawSheet_(startRow, rows, bufferTexts, bufferColors, extraCells) {
  ...
  (extraCells || []).forEach((cell, index) => {
    ...
    if (cleanCellText_(cell?.price)) {
      data.push({
        range: `'${escapeSheetName_(CONFIG.RAW_SHEET_NAME)}'!B${rowNumber}:B${rowNumber}`,
        values: [[cleanCellText_(cell.price)]]
      });
    }
    if (cleanCellText_(cell?.imageUrl)) {
      data.push({
        range: `'${escapeSheetName_(CONFIG.RAW_SHEET_NAME)}'!C${rowNumber}:C${rowNumber}`,
        values: [[cleanCellText_(cell.imageUrl)]]
      });
    }
    if (cleanCellText_(cell?.pickupDate)) {
      data.push({
        range: `'${escapeSheetName_(CONFIG.RAW_SHEET_NAME)}'!J${rowNumber}:J${rowNumber}`,
        values: [[cleanCellText_(cell.pickupDate)]]
      });
    }
  });
  ...
  for (const target of formulaTargets) {
    const templateFormula = await findNearestFormulaAbove_(CONFIG.RAW_SHEET_NAME, startRow, target.col);
    if (!templateFormula) continue;
...
function buildParserPrompt_(params) {
  return JSON.stringify({
    ...
    strictRules: [
      ...
      '잠원메이플자이점 부점장, 잠원메이플자이점 점장, 점장, 부점장, 운영자, 관리자 메시지는 ignore 처리한다.',
...
function normalizeAiEventType_(event) {
  ...
  if (/잠원메이플자이점\s*(부점장|점장)|부점장|점장|운영자|관리자/.test(String(event.user || ''))) {
