...
export function aggregateDashboardRows(rows, options = {}) {
  ...
  const period = resolvePeriod(rows, mode, options, today);
  const periodRows = filterRowsForPeriod(rows, period);
  const allTotals = buildTotals(rows);
  ...
  return {
    ...
    options: buildOptions(rows, today),
    meta: {
      mode,
...
function resolvePeriod(rows, mode, options, today) {
  if (mode === 'total') {
  ...
  if (mode === 'week') {
    const year = parseIntOr(options.year, today.getFullYear());
    const month = parseIntOr(options.month, today.getMonth() + 1);
    const ranges = getMonthWeekRanges(year, month);
    const weekIndex = clamp(parseIntOr(options.weekIndex, getCurrentWeekIndex(ranges, today)), 1, ranges.length);
    const selected = ranges[weekIndex - 1] || ranges[0];

    return {
      label: `${selected.label} (${selected.rangeLabel})`,
      from: selected.from,
      to: selected.to
    };
  }
  ...
  if (mode === 'custom') {
    const from = parseDashboardDate(options.from, today) || today;
    const to = parseDashboardDate(options.to, today) || from;
