...
export function parseDashboardRows(rows, options = {}) {
  ...
  const excludedCustomerSet = buildExcludedCustomerSet(options.excludedCustomerNames || []);
  const warnings = [];
  const validRows = [];
  let excludedAllyRowCount = 0;
  ...
  (rows || []).forEach((row, rowIndex) => {
    ...
    const warningBase = {
      rowNumber,
      basisDate: basisDate ? toDateKey(basisDate) : '',
      groupDate: clean(raw.groupDate),
      orderDate: clean(raw.orderDate),
      pickupDate: clean(raw.pickupDate),
    ...
    if (isExcludedCustomer(customerName, excludedCustomerSet)) {
      excludedAllyRowCount += 1;
      return;
    }
  ...
  return { validRows, warnings, excludedAllyRowCount };
}
...
export function normalizeCustomerNameForExclusion(value) {
  return clean(value)
    .toLowerCase()
    .replace(/\s+/g, '');
}

function buildExcludedCustomerSet(names) {
  return new Set(
    names
      .map(normalizeCustomerNameForExclusion)
      .filter(Boolean)
  );
}

function isExcludedCustomer(customerName, excludedCustomerSet) {
  if (!customerName || !excludedCustomerSet.size) return false;

  return excludedCustomerSet.has(normalizeCustomerNameForExclusion(customerName));
}
