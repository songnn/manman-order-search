...
export async function searchOrders(keyword, selectedCustomerLabel) {
  ...
  const pickupCompletionMapPromise = prefetchPickupCompletionMapForKeyword_(keyword).catch(error => {
    console.warn('pickup completion prefetch skipped:', error.message);
    return null;
  });
  ...
  const filteredRows = hasSelectableCandidates
    ...
    : visibleMatchedRows;
  const prefetchedPickupCompletionMap = await pickupCompletionMapPromise;
  const pickupCompletionMap = prefetchedPickupCompletionMap || await getPickupCompletionMap(filteredRows.map(row => ({
...
async function prefetchPickupCompletionMapForKeyword_(keyword) {
  const cleanedKeyword = clean_(keyword);
  const keywordDigits = cleanedKeyword.replace(/\D/g, '');

  if (keywordDigits.length < 4) return null;

  const { data, error } = await supabaseAdmin
    .from('pickup_completions')
    .select('source_sheet_name,source_row_number,completed,completed_at')
    .eq('store_name', CONFIG.STORE_NAME)
    .eq('customer_digits4', keywordDigits.slice(-4))
    .limit(Number(process.env.PICKUP_COMPLETION_PREFETCH_MAX_ROWS || 1000));

  if (error) {
    if (isMissingPickupCompletionTable_(error)) return null;
    throw error;
  }

  return new Map((data || []).map(row => [
    `${row.source_sheet_name}::${row.source_row_number}`,
    row
  ]));
}

function isMissingPickupCompletionTable_(error) {
