...
import { parseDashboardRows, toDateKey } from './dashboard/parseOrders.js';

export const KAKAO_SHEETS = {
  uploads: process.env.KAKAO_CSV_UPLOAD_SHEET_NAME || 'Kakao_CSV_Uploads',
  messages: process.env.KAKAO_CSV_MESSAGE_SHEET_NAME || 'Kakao_CSV_Messages',
  events: process.env.KAKAO_CSV_EVENT_SHEET_NAME || 'Kakao_CSV_MemberEvents',
  matches: process.env.KAKAO_CSV_MATCH_SHEET_NAME || 'Kakao_CSV_OrderMatches'
};
...
const RAW_READ_START_ROW = Number(process.env.ADMIN_DASHBOARD_READ_START_ROW || process.env.RAW_READ_START_ROW || 1);
...
export async function ingestKakaoCsvUpload(body) {
  ...
  const windowMessages = parsedMessages.filter(message => isInsideWindow(message, meta));
  const memberEvents = extractMemberEvents(windowMessages, meta);
  const shouldMatchRawOrdersOnUpload = process.env.KAKAO_CSV_MATCH_ON_UPLOAD === '1';
  ...
  const uploadSummary = buildUploadSummary(meta, parsedMessages, windowMessages, memberEvents, analysis);
  ...
  debugKakaoCsvStep('sheets:client:start');
  const sheets = await getSheetsClient();
  debugKakaoCsvStep('sheets:client:done');
  debugKakaoCsvStep('sheets:ensure:start');
  await ensureKakaoSheets(sheets);
  debugKakaoCsvStep('sheets:ensure:done');
  debugKakaoCsvStep('sheets:text-format:start');
  await formatKakaoSheetsAsText(sheets);
  debugKakaoCsvStep('sheets:text-format:done');
  debugKakaoCsvStep('sheets:clear:start');
  await clearKakaoDataRows(sheets);
  debugKakaoCsvStep('sheets:clear:done');
  debugKakaoCsvStep('sheets:write:start');
  await writeKakaoUploadData(sheets, {
    messages: windowMessages,
    memberEvents,
    matches: analysis.matches,
    uploads: [uploadSummary]
  });
  debugKakaoCsvStep('sheets:write:done');
...
export async function readKakaoCsvStore() {
  try {
    const sheets = await getSheetsClient();
    const [uploads, messages, events, matches] = await Promise.all([
      readSheetObjects(sheets, KAKAO_SHEETS.uploads),
      readSheetObjects(sheets, KAKAO_SHEETS.messages),
      readSheetObjects(sheets, KAKAO_SHEETS.events),
      readSheetObjects(sheets, KAKAO_SHEETS.matches)
    ]);
...
export async function reanalyzeKakaoCsvMatches(options = {}) {
  ...
  for (const upload of uploads) {
    ...
    const windowMessages = messages
      .map(messageToPipelineObject)
      .filter(message => isInsideWindow(message, meta));

    await removeRowsByFirstColumn(sheets, KAKAO_SHEETS.matches, upload.uploadId);
    await appendObjects(sheets, KAKAO_SHEETS.matches, KAKAO_HEADERS.matches, analysis.matches);
    await upsertUploadSummary(sheets, buildUploadSummary(meta, messages, windowMessages, memberEvents, analysis));
...
async function readRawOrdersForUpload(meta) {
  ...
  const response = await sheets.spreadsheets.values.get({
    ...
    range: `${RAW_SHEET_NAME}!A${Math.max(1, RAW_READ_START_ROW)}:J`,
    valueRenderOption: 'UNFORMATTED_VALUE',
    dateTimeRenderOption: 'SERIAL_NUMBER'
  });
  const parsed = parseDashboardRows(response.data.values || [], {
    startRowNumber: Math.max(1, RAW_READ_START_ROW),
    basis: 'groupDate'
  });

  return buildRawOrders(parsed.validRows, meta)
    .filter(order => !meta.orderDate || order.orderDate === meta.orderDate);
}

function buildRawOrders(rows, meta) {
  const occurrence = new Map();

  return (rows || [])
    .map(row => {
      const orderDate = toDateKey(row.groupDate || row.orderDate || row.basisDate);
      const normalizedCustomer = normalizeName(row.customerName);
      const normalizedProduct = normalizeProduct(row.productName);
      ...
      return {
        ...
        quantity,
        occurrenceIndex,
        currentSourceRowNumber: row.rowNumber,
        sourceSheetName: RAW_SHEET_NAME
      };
    })
    .filter(order => order.normalizedCustomer && order.normalizedProduct && order.quantity > 0)
    .sort((a, b) => a.currentSourceRowNumber - b.currentSourceRowNumber);
}
...
function analyzeAndMatchOrders({ meta, messages, rawOrders, memberEvents }) {
  ...
  messages.forEach(message => {
    if (message.message_type !== 'join' && message.message_type !== 'leave' && isOrderLikeMessage(message, rawOrders)) {
...
function isOrderLikeMessage(message, rawOrders) {
  ...
  return rawOrders.some(order => productTokens(order.productName).some(token => normalized.includes(token)));
}
...
function buildUploadSummary(meta, allMessages, windowMessages, memberEvents, analysis) {
  return {
    ...
    first_order_after_minutes: analysis.firstOrderAfterMinutes,
    notes: '원본 CSV/TXT 서버 분석'
  };
}

async function ensureKakaoSheets(sheets) {
  ...
  await sheets.spreadsheets.values.batchUpdate({
    ...
    requestBody: {
      data: Object.entries(KAKAO_SHEETS).map(([key, sheetName]) => {
        const headers = KAKAO_HEADERS[key];
        return {
...
async function formatKakaoSheetsAsText(sheets) {
  const spreadsheetId = getSpreadsheetId();
  const response = await sheets.spreadsheets.get({
    spreadsheetId,
    fields: 'sheets.properties'
  ...
  const sheetIdsByTitle = new Map(
    (response.data.sheets || [])
      ...
      .filter(([title, sheetId]) => title && sheetId != null)
  );
  const requests = Object.entries(KAKAO_SHEETS)
    .map(([key, sheetName]) => {
      const sheetId = sheetIdsByTitle.get(sheetName);
      ...
      return {
        repeatCell: {
          range: {
            ...
            startRowIndex: 0,
            startColumnIndex: 0,
            endColumnIndex: KAKAO_HEADERS[key].length
          },
          cell: {
            userEnteredFormat: {
              numberFormat: {
...
async function writeKakaoUploadData(sheets, data) {
  const spreadsheetId = getSpreadsheetId();
  const sheetRows = {
    uploads: data.uploads || [],
    messages: data.messages || [],
    events: data.memberEvents || [],
  ...
  await sheets.spreadsheets.values.batchUpdate({
    ...
    requestBody: {
      data: Object.entries(KAKAO_SHEETS).map(([key, sheetName]) => {
        ...
        const rows = [
          headers,
          ...(sheetRows[key] || []).map(object => headers.map(header => object[header] ?? ''))
        ];

        return {
          range: `${sheetName}!A1:${columnName(headers.length)}${rows.length}`,
          values: rows
        };
      })
    }
...
async function clearKakaoDataRows(sheets) {
  await Promise.all(Object.values(KAKAO_SHEETS).map(sheetName =>
    clearSheetDataRows(sheets, sheetName)
  ));
}

async function upsertUploadSummary(sheets, summary) {
  await removeRowsByFirstColumn(sheets, KAKAO_SHEETS.uploads, summary.upload_id);
  await appendObjects(sheets, KAKAO_SHEETS.uploads, KAKAO_HEADERS.uploads, [summary]);
}

async function clearSheetDataRows(sheets, sheetName) {
...
async function removeRowsByFirstColumn(sheets, sheetName, value) {
  const spreadsheetId = getSpreadsheetId();
  ...
  if (!rows.length) return;
  const keptRows = rows.filter(row => String(row?.[0] || '') !== value);
  if (keptRows.length === rows.length) return;

  await sheets.spreadsheets.values.clear({ spreadsheetId, range });
  if (keptRows.length) await appendRows(sheets, sheetName, keptRows);
}

async function appendObjects(sheets, sheetName, headers, objects) {
...
async function readSheetObjects(sheets, sheetName) {
  const expectedHeaders = getKakaoHeadersForSheet(sheetName);
  const endColumn = columnName(expectedHeaders?.length || 52);
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: getSpreadsheetId(),
...
function normalizeUploadRecord(row) {
  return {
    ...
    firstOrderAfterMinutes: toNumber(row.first_order_after_minutes),
    notes: clean(row.notes)
  };
}

function getKakaoHeadersForSheet(sheetName) {
  const entry = Object.entries(KAKAO_SHEETS)
    .find(([, configuredSheetName]) => configuredSheetName === sheetName);
  if (!entry) return null;
  return KAKAO_HEADERS[entry[0]] || null;
}
...
function eventToSheetObject(event) {
  return {
