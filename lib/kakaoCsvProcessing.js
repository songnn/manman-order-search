...
export async function readKakaoCsvStore(options = {}) {
  try {
    const maxUploads = Math.max(1, Number(options.maxUploads || 20));
    const uploads = await readSupabaseRows(KAKAO_TABLES.uploads, query =>
      query.select('*').order('uploaded_at', { ascending: false }).limit(maxUploads),
      { paginate: false }
    );
...
export async function reanalyzeKakaoCsvMatches(options = {}) {
  ...
  for (const upload of uploads) {
    ...
    if (upload.messageCount && messages.length < upload.messageCount) {
      console.warn(
        `Kakao CSV reanalysis skipped for ${upload.uploadId}: expected ${upload.messageCount} messages, got ${messages.length}`
      );
    ...
    if (!rawOrders.length && (upload.rawOrderCount > 0 || upload.matchedOrderCount > 0)) {
      console.warn(
        `Kakao CSV reanalysis skipped for ${upload.uploadId}: raw order cache returned 0 rows`
      );
      skippedUploadCount += 1;
      continue;
    }
  ...
  return { ok: true, uploadCount: uploads.length, skippedUploadCount, matchCount: totalMatches };
}
