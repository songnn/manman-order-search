...
export function getKakaoCsvExpectedToken() {
  return process.env.KAKAO_CSV_INGEST_TOKEN || process.env.ADMIN_DASHBOARD_TOKEN || '';
}
...
export async function ingestKakaoCsvUpload(body) {
  ...
  const parsedMessages = parseKakaoMessages(fileContent).map((message, index, allMessages) =>
    normalizeParsedMessage(message, index, allMessages, meta)
  );
  ...
  await replaceUploadRows(sheets, uploadId);

  await appendObjects(sheets, KAKAO_SHEETS.messages, KAKAO_HEADERS.messages, parsedMessages);
  await appendObjects(sheets, KAKAO_SHEETS.events, KAKAO_HEADERS.events, memberEvents);
  await appendObjects(sheets, KAKAO_SHEETS.matches, KAKAO_HEADERS.matches, analysis.matches);
  await appendObjects(sheets, KAKAO_SHEETS.uploads, KAKAO_HEADERS.uploads, [
    buildUploadSummary(meta, parsedMessages, windowMessages, memberEvents, analysis)
  ]);
...
function normalizeParsedMessage(row, index, allMessages, meta) {
  const date = parseKakaoDate(row.dateRaw);
  const sameBaseIndex = allMessages
    .slice(0, index + 1)
    .filter(item =>
      clean(item.dateRaw) === clean(row.dateRaw) &&
      clean(item.user) === clean(row.user) &&
      clean(item.message) === clean(row.message)
    ).length;
...
async function ensureKakaoSheets(sheets) {
  for (const [key, sheetName] of Object.entries(KAKAO_SHEETS)) {
    await ensureSheetWithHeaders(sheets, sheetName, KAKAO_HEADERS[key]);
  }
}
...
async function replaceUploadRows(sheets, uploadId) {
  await removeRowsByFirstColumn(sheets, KAKAO_SHEETS.uploads, uploadId);
  await removeRowsByFirstColumn(sheets, KAKAO_SHEETS.messages, uploadId);
  await removeRowsByFirstColumn(sheets, KAKAO_SHEETS.events, uploadId);
  await removeRowsByFirstColumn(sheets, KAKAO_SHEETS.matches, uploadId);
}
