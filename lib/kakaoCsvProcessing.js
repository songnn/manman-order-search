...
export async function reanalyzeKakaoCsvMatches(options = {}) {
  ...
  for (const upload of uploads) {
    ...
    await updateSupabaseUploadSummary(buildUploadSummary(meta, messages, windowMessages, memberEvents, analysis));
...
async function readRawOrdersForUpload(meta) {
  ...
  if (orderDateValue) {
    query = query.eq('order_date_value', orderDateValue);
  }

  const { data, error } = await query;
  if (error) throw error;

  return buildRawOrdersFromOrderCache(data || [], meta);
}
...
async function replaceKakaoUploadInSupabase({ uploadSummary, messages, memberEvents, matches }) {
  ...
  await insertSupabaseRows(KAKAO_TABLES.messages, messages);
  await insertSupabaseRows(KAKAO_TABLES.events, memberEvents);
  await insertSupabaseRows(KAKAO_TABLES.matches, matches);
}
...
async function insertSupabaseRows(tableName, rows, chunkSize = 500) {
  if (!rows.length) return;
  ...
  for (let i = 0; i < dbRows.length; i += chunkSize) {
    const { error } = await supabaseAdmin
      .from(tableName)
      .insert(dbRows.slice(i, i + chunkSize));
