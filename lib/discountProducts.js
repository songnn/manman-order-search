...
const DISCOUNT_SHEET_RANGE = "'할인상품'!A2:K";
...
export async function getActiveDiscountProducts({
  ...
  if (preferSnapshot) {
    ...
    const snapshotResult = getActiveDiscountProductsFromSnapshot(nowKst);
    if (snapshotResult) {
...
function getActiveDiscountProductsFromSnapshot(nowKst) {
...
function refreshDiscountRowsInBackground() {
  ...
  backgroundRowsRefreshPromise = readDiscountRows({ force: true })
    .catch(error => {
